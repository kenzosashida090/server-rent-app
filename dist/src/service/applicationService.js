...
export const listApplicationsDB = async (userId, userType) => {
    try {
        let whereClauses = {};
        if (userId && userType) {
            if (userType === "tenant") {
        ...
        const formattedApplication = await Promise.all(application.map(async (app) => {
...
export const createApplicationDB = async (body) => {
    try {
        const { applicationDate, status, propertyId, tenantCognitoId, name, email, phoneNumber, message, } = body;
        ...
        const newApplication = await prisma.$transaction(async (primsa) => {
            ...
            const lease = await primsa.lease.create({
                data: {
                    ...
                    endDate: new Date(new Date().setFullYear(new Date().getFullYear() + 1)), // hardcoded
...
export const updatingApplicationStatusDB = async (id, status) => {
    try {
