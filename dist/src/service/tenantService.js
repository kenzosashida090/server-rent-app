...
export const getTenantService = async (cognitoId) => {
...
export const updateTenantService = async (cognitoId, data) => {
    const tenant = await prisma.tenant.update({
        where: { cognitoId }, data
    });
    return tenant;
};
export const getTenantPropertiesDB = async (tenantId) => {
    try {
...
export const addFavoritePropertyDB = async (data) => {
    try {
...
export const removeFavoritePropertyDB = async (data) => {
    try {
        const propertyIdNumber = Number(data.propertyId);
        const updateTenant = await prisma.tenant.update({
            where: { cognitoId: data.cognitoId },
