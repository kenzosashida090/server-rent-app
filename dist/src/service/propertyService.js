...
export const getPropertiesDB = async (query) => {
    try {
    }
    catch (error) {
        const { favoriteIds, priceMin, priceMax, beds, baths, propertyType, squareFeetMin, squareFeetMax, amenities, availableFrom, latitude, longitude } = query;
        let whereConditions = [];
        if (favoriteIds) {
            const favoiriteIdsArray = favoriteIds.split(",").map(Number);
            whereConditions.push(Prisma.sql `p.id IN (${Prisma.join(favoiriteIdsArray)})`);
        }
        if (priceMin) {
...
export const getPropertyDB = async (id) => {
    try {
...
export const createPropertyDB = async (data, images) => {
